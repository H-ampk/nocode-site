...
## 概要
ノーコード教材エディタに評価軸ベクトル機能と分析機能を完全統合しました。エディタで選択肢にベクトルを設定し、プレイヤーの回答ログにベクトルを記録し、分析画面で理想ベクトルと実績ベクトルの類似度を計算・表示できるようになりました。

## 改善対象

### 1. エディタでの選択肢ベクトル保存
### 2. プレイヤーでの回答ログへのベクトル追加
### 3. データローダーでのproject.values読み込み
### 4. ベクトル分析機能の実装
### 5. 分析画面へのベクトル分析タブ追加

## 実装内容

### 1. editor.js - 選択肢のvectorをquiz.jsonに保存

#### 保存処理の追加
- `saveProject()`関数で、保存前に選択肢の`vector`プロパティを設定
- `question.vector_scores`から各選択肢の`vector`をコピー
- 選択肢オブジェクトに直接`vector`プロパティを追加
...
### 2. player.js - 回答ログにvectorを追加

#### ログ形式の統一
- `confirmAnswer()`関数で、選択肢の`vector`をログエントリに追加
- `quiz_log_dummy.json`形式と完全互換にするため、ログ形式を統一
  - `questionId`: 問題ID
  - `final_answer`: 最終回答
  - `correct`: 正誤
  - `response_time`: 反応時間
  - `path`: 選択肢遷移のシーケンス
  - `vector`: 選択肢のベクトル（新規追加）
...
### 3. dataset_loader.js - project.valuesの読み込み追加

#### loadProject関数の追加
- `project.json`から`values`（理想ベクトル）を読み込む関数を追加
- プロジェクト設定を返す際に`values`を含める
- エラー時は空のオブジェクトを返す（後方互換性のため）
...
### 4. analysis/metrics/vector.js - 新規作成

#### ベクトル変換と類似度計算関数
- `toVector()`: オブジェクト形式のベクトルを配列形式に変換
- `cosineSimilarity()`: 2つのベクトルのコサイン類似度を計算
- ES5形式（IIFE）で実装し、グローバルに`VectorMetrics`として公開
...
### 5. analysis.html - ベクトル分析タブとUIセクションの追加

#### タブの追加
- 「ベクトル分析」タブを追加（Glossary提示履歴タブの後）
- タブ切り替え機能に統合

#### UIセクションの追加
- 理想構造との一致度（cosine similarity）を表示するメトリックボックス
- 学習者の平均ベクトルを表示するリスト

```html
<div id="tab-vector" class="tab-content">
    <h3>理解ベクトル分析</h3>
    <div id="vector-analysis">
        <div class="metric-box" style="background: #f9f9f9; padding: 1.5rem; border-radius: 8px; margin-bottom: 2rem; border-left: 4px solid #2196F3;">
            <strong style="display: block; margin-bottom: 0.5rem; color: #333;">理想構造との一致度（cosine similarity）</strong>
            <div id="vector-similarity" style="font-size: 2rem; font-weight: bold; color: #2196F3;">--%</div>
        </div>

        <h4 style="margin-top: 2rem; color: #333; border-bottom: 2px solid #eee; padding-bottom: 0.5rem;">学習者の平均ベクトル</h4>
        <ul id="vector-axis-list" style="list-style: none; padding: 0; margin: 1rem 0;"></ul>
    </div>
</div>
```

### 6. analysis.js - ベクトル集計ロジックの追加

#### renderVectorAnalysis関数の実装
- ログから`vector`を集計し、平均ベクトルを計算
- 理想ベクトル（`project.values`）と実績ベクトルを比較
- コサイン類似度を計算してUIに表示
- 各評価軸の平均値をリスト表示
...
#### renderAll関数の更新
- `renderAll()`関数に`projectData`パラメータを追加
- ベクトル分析を自動実行（`projectData`がある場合、またはデフォルトプロジェクトを読み込む）

#### analyze関数の更新
- `analyze()`関数に`projectData`パラメータを追加
- データセットとプロジェクト設定を並行して読み込む

### 7. analysis.html - データ読み込み処理の更新

#### loadAndAnalyze関数の更新
- データセットとプロジェクト設定を`Promise.all()`で並行読み込み
- `AnalysisDashboard.analyze()`に`projectData`を渡す

```javascript
// データセットとプロジェクト設定を並行して読み込む
Promise.all([
    DatasetLoader.loadDataset(dataset),
    DatasetLoader.loadProject ? DatasetLoader.loadProject('default') : Promise.resolve({})
])
    .then(function(results) {
        ...
        // 分析を実行（projectDataを含める）
        AnalysisDashboard.analyze(logs, projectData);
    });
```
...
### 2. quiz_log_dummy.json形式との完全互換
- ログ形式を統一（`questionId`, `final_answer`, `correct`, `response_time`, `path`, `vector`）
- 既存のログファイルと互換性を維持
- ベクトルデータが含まれていないログにも対応

### 3. 理想ベクトルとの比較機能
- `project.json`の`values`から理想ベクトルを読み込み
- コサイン類似度で理想と実績を比較
- 視覚的に分かりやすいUIで表示

### 4. モジュール化されたベクトル計算
- `analysis/metrics/vector.js`として独立したモジュール
- `toVector()`と`cosineSimilarity()`を再利用可能な関数として実装
- ES5形式で既存コードと互換性を維持

### 5. 自動的なベクトル分析実行
- データセット読み込み時に自動的にベクトル分析を実行
- プロジェクト設定がない場合はデフォルトプロジェクトを読み込む
- エラーハンドリングを実装

## 更新ファイル

### editor.js
- `saveProject()`関数に選択肢の`vector`保存処理を追加

### src/player/player.js
- `confirmAnswer()`関数にベクトルをログに追加する処理を実装
- ログ形式を`quiz_log_dummy.json`と完全互換にする

### src/admin/dataset_loader.js
- `loadProject()`関数を追加（`project.json`から`values`を読み込み）

### analysis/metrics/vector.js
- **新規作成**: `toVector()`と`cosineSimilarity()`関数を実装

### admin/analysis.html
- 「ベクトル分析」タブを追加
- ベクトル分析UIセクションを追加
- `loadAndAnalyze()`関数を更新（プロジェクト設定も読み込む）

### src/admin/analysis.js
- `renderVectorAnalysis()`関数を実装
- `renderAll()`関数に`projectData`パラメータを追加
- `analyze()`関数に`projectData`パラメータを追加
...
### データフローの統一
- エディタ → プレイヤー → 分析画面の一貫したデータフロー
- ベクトルデータが各段階で正しく受け渡される
- ログ形式の統一により、既存データとの互換性を維持

### モジュール化された設計
- ベクトル計算ロジックを独立したモジュールとして実装
- 再利用可能な関数設計
- ES5形式で既存コードと互換性を維持

### エラーハンドリング
- プロジェクト設定がない場合のフォールバック処理
- ベクトルデータがないログへの対応
- 空のベクトルや不正なデータ構造への対応

### パフォーマンス
- データセットとプロジェクト設定の並行読み込み
- 効率的なベクトル集計アルゴリズム
- UI更新の最適化
...
- 既存の分析タブと統一されたデザイン
- タブ切り替えで簡単にアクセス可能
- データ読み込み時に自動的にベクトル分析を実行

### 教育者向けの配慮
- 理想ベクトルとの比較で学習者の理解度を可視化
- コサイン類似度で数値的に評価
- 各評価軸の平均値を個別に表示
...
### 1. エディタでのベクトル設定
1. エディタで質問を編集
2. 選択肢ごとに評価軸ベクトルを設定（-1/0/+1）
3. プロジェクトを保存すると、選択肢の`vector`が`quiz.json`に保存される

### 2. プレイヤーでのベクトル記録
1. プレイヤーが選択肢を選択
2. 回答確定時に、選択肢の`vector`がログに追加される
3. ログファイルに`vector`フィールドが含まれる

### 3. 分析画面でのベクトル分析
1. データセットを選択
2. プロジェクト設定（理想ベクトル）を自動読み込み
3. ログからベクトルを集計し、平均ベクトルを計算
4. 理想ベクトルと実績ベクトルのコサイン類似度を計算
5. 「ベクトル分析」タブで結果を表示

## 備考 / 今後のタスク

- ベクトル分析のグラフ表示機能（将来的に）
- 複数の理想ベクトルとの比較機能（将来的に）
- ベクトルの時系列分析機能（将来的に）
- ベクトル分析結果のエクスポート機能（将来的に）
