...
## AnalysisDashboard の公開修正とスクリプト読み込み順序の改善（追加改善）

### src/admin/analysis.js
- **`global.AnalysisDashboard` の定義位置を修正**
  - 以前: 1237行目（`computeJSONDiff` と `renderJSONDiff` の定義前）
  - 現在: 1465行目（IIFEの終了直前、すべての関数定義の後）
  - これにより、すべての関数が定義された後に `AnalysisDashboard` が公開されるようになった
- **IIFE構造の確認**
  - IIFEは `(function (global) { ... })(window);` の形式で正しく閉じられている
  - `global.AnalysisDashboard` はIIFE内で定義されている
- **公開されている関数の確認**
  - `loadQuizLog`, `mergeAllSessions`, `computeOverallStats`, `computePerQuestionStats`, `computeConceptConfusions`, `computeResponseTimeProfile`, `computePathPatterns`, `computeGlossaryUsage`, `computeVectorStats`
  - `renderOverallStats`, `renderQuestionStats`, `renderConfusionStats`, `renderResponseTimeStats`, `renderPathStats`, `renderGlossaryStats`, `renderVectorAnalysis`, `renderAll`, `renderAllWithProject`
  - `analyze`, `getQuizVersionsFromLogs`, `filterLogsByVersion`, `runClusterAnalysis`, `convertLogsToCSV`, `renderClusterAnalysis`, `computeJSONDiff`, `renderJSONDiff`, `escapeHtml`
  - すべての主要関数が正しく公開されている

### admin/analysis.html
- **スクリプト読み込み順序の修正**
  - `analysis.js` を最初に読み込むように修正（664行目）
  - `AnalysisDashboard` を使用するスクリプト（671行目以降）の前に配置
  - `<body>` の一番下（1595行目）にも配置して確実に読み込まれるようにした
  - 確認用のスクリプトを追加（`window.AnalysisDashboard` の存在確認）
- **`type="module"` の扱い**
  - `vector_math.js` の読み込み部分を `type="text/javascript"` に変更
  - 動的インポートはES modulesでのみ動作するため、コメントのみに変更

## 改善の詳細（AnalysisDashboard の公開修正）

### スクリプト読み込み順序の重要性
- `AnalysisDashboard` を使用するスクリプトより前に `analysis.js` を読み込む必要がある
- `<script>` タグは順次実行されるため、読み込み順序が重要
- `<body>` の一番下に配置することで、DOMが読み込まれた後に確実に実行される

### IIFE構造の整合性
- すべての関数をIIFE内で定義し、最後に `global.AnalysisDashboard` として公開
- これにより、グローバルスコープの汚染を防ぎつつ、必要な関数を公開できる
- 関数の定義順序と公開順序を一致させることで、参照エラーを防止

### エラー解決
- "AnalysisDashboard is not defined" エラーを解消
- コンソールで `console.log(window.AnalysisDashboard)` を実行すると、オブジェクトが正しく表示される
- データセットを選択したときに全体概要などが正しく表示される

## フロントエンド全体の整合性確保とクラスタリング分析システムの完全統合（追加改善）

### 概要
analysis.js と analysis.html の包括的なデバッグ・リファクタリングを行い、スクリプト読み込み順序の正規化、ES module の排除、イベントリスナーの重複登録防止、A+B統合版クラスタリング分析システムの実装を完了しました。

### src/utils/vector_math.js
- **ES module から IIFE + window 形式に変更**
  - `export function cosineSimilarity()` → `(function (global) { ... })(window)` 形式に変更
  - `window.VectorMath.cosineSimilarity` として公開
  - ES module 禁止ルールに準拠

### src/admin/dataset_loader.js
- **`loadDataset()` の戻り値を標準形に統一**
  - 標準形: `{ user_id, session_id, quiz_version, logs, raw }`
  - multi-session / single-export / class-data のバラつきを吸収
  - `logs` 配列に統一（AnalysisDashboard 側が壊れないように）
  - 元データは `raw` に保持
  - `vector_test_sessions` や `sessions` 配列から自動的に `logs` を抽出

### admin/analysis.html
- **スクリプト読み込み順序を正規化（絶対に変更禁止）**
  - 正しい順序:
    1. `config.js`
    2. `GlossaryLoader.js`
    3. `dataset_loader.js`
    4. `vector_math.js`
    5. `analysis.js`（1回のみ）
    6. main script（この中で AnalysisDashboard を使う）
  - 重複読み込みを削除（以前は2回読み込まれていた）
  - 読み込み確認用のコンソールログを追加

### src/admin/analysis.js
- **A+B統合版 `renderClusterAnalysis()` を実装**
  - **A: Julia 出力優先読み込み**
    - `../../analysis/cluster_output.json` を読み込み
    - `../../analysis/cluster_scatter.png` を読み込み
    - 存在する場合は Julia 出力を優先表示
  - **B: JS fallback 自動実行**
    - Julia 出力が存在しない場合は自動的に JS fallback を実行
    - 簡易 k-means クラスタリング（`simpleKMeans()`）を実行
    - Chart.js で散布図を描画
  - **実装した関数:**
    - `renderClusterAnalysis(datasetData, projectId)`: メイン関数
    - `renderClusterAnalysisWithJuliaOutput()`: Julia出力表示
    - `renderClusterAnalysisWithJSFallback()`: JS fallback実行
    - `simpleKMeans(features, k)`: 簡易 k-means クラスタリング
    - `euclideanDistance(a, b)`: ユークリッド距離計算
    - `renderClusterScatter(features, labels, sessionInfo)`: Chart.js で散布図描画（改善版）
    - `renderClusterTable(labels, sessionInfo)`: クラスタテーブル表示（JS fallback用）
    - `renderClusterTableWithJuliaOutput(results, sessionMap, clusterStats)`: クラスタテーブル表示（Julia出力用）
- **イベントリスナーの重複登録防止**
  - すべてのイベントリスナーに重複登録防止フラグを追加
  - `window._runClusterBtnListenerAdded`, `window._clusterAnalysisBtnListenerAdded` など
- **`window.currentLogs` の確実なセット**
  - `analyze()` と `renderAll()` 内で `window.currentLogs` をセット
  - `analysis-run` タブで使用可能に
- **VectorMetrics → VectorMath への変更**
  - `window.VectorMetrics.cosineSimilarity` → `window.VectorMath.cosineSimilarity`
  - `window.VectorMetrics.toVector` → 直接実装（オブジェクトから配列へ変換）

### admin/analysis.html
- **クラスタリング呼び出し箇所を統合版に統一**
  - `window.AnalysisDashboard.renderClusterAnalysis(data.raw, projectData.project_id || 'default')` に統一
  - `data.raw.student_log` にも対応
- **エラーハンドリングの強化**
  - `AnalysisDashboard` と `DatasetLoader` の存在確認を追加
  - エラー発生時もアプリケーションが停止しないように改善
- **すべての `escapeHtml` 参照を統一**
  - `window.AnalysisDashboard.escapeHtml` を使用

## 改善の詳細（フロントエンド全体の整合性確保）
...
- この順序を破ると `AnalysisDashboard`, `DatasetLoader`, `VectorMath` が未定義になる

### ES module の完全排除
- `<script type="module">` を禁止
- すべて IIFE + `window.*` 形式で export
- `globalThis` ではなく `window` を使用
- `vector_math.js` を ES module から IIFE 形式に変更

### イベントリスナーの重複登録防止
- すべての `addEventListener` に重複登録防止フラグを追加
- `window._*ListenerAdded` フラグで制御
- ページ初期化関数 `init()` の呼び出しを1回に統一
- タブ UI / diff UI / cluster UI / RT fitting UI のイベントは1回のみ登録

### データ形式の統一
- `dataset_loader.js` の `loadDataset()` が標準形を返すように統一
- `AnalysisDashboard` 側が壊れないように設計
- multi-session / single-export / class-data のバラつきを吸収

### A+B統合版クラスタリング分析システム
- **2段階フォールバック方式:**
  1. **A: Julia 出力優先**
     - `cluster_output.json` と `cluster_scatter.png` を読み込み
     - 存在する場合は Julia 出力を優先表示
     - クラスタ統計とセッション詳細を表示
  2. **B: JS fallback 自動実行**
     - Julia 出力が存在しない場合は自動的に JS fallback を実行
     - 簡易 k-means クラスタリング（k=3）を実行
     - Chart.js で散布図を描画
     - クラスタテーブルを表示
- **エラーハンドリング:**
  - 各ステップで null チェックを実装
...
2. データセットを選択（`cluster_dummy.json` など、`cluster_features` を含むデータ）
3. 自動的にクラスタリング分析が実行される

### 2. Julia 分析（オプション）
1. `runClusterBtn` をクリックして CSV をダウンロード
2. `julia analysis/clustering.jl students/cluster_dummy.json analysis/cluster_output.json` を実行
3. `analysis/cluster_scatter.png` を生成（オプション）
4. ページをリロードすると Julia 出力が優先表示される

### 3. JS fallback（自動）
- Julia 出力が存在しない場合、自動的に JS fallback が実行される
- 簡易 k-means クラスタリングが実行され、散布図とテーブルが表示される

## 動作確認項目

以下の動作を確認してください：

1. ✅ スクリプト読み込み順序が正しい（コンソールで確認）
2. ✅ `AnalysisDashboard`, `DatasetLoader`, `VectorMath` が正しく読み込まれる
3. ✅ データセット選択 → 分析が正常に動作する
4. ✅ クラスタリング分析が自動実行される
5. ✅ Julia 出力があれば優先表示、なければ JS fallback が実行される
6. ✅ 散布図とクラスタテーブルが表示される
7. ✅ コンソールエラーがゼロである
8. ✅ "renderClusterAnalysis is not defined" エラーが発生しない

## 旧ベクトル解析コードの完全削除と構造統一（追加改善）

### 概要
analysis.js 内に残存していた旧ベクトル解析関数（`renderVectorAnalysis`）を完全に削除し、`computeVectorStats()` と `vector_math.js` に統合済みの関数のみを使用する構造に統一しました。未定義変数 `actualVector`, `idealVector` を参照していたバグも解消しました。

### src/admin/analysis.js
- **旧関数 `renderVectorAnalysis()` の完全削除**
  - 748-838行目にあった旧実装を完全削除
  - `actualVector` 未定義変数を参照していたバグを含む
  - `computeVectorStats()` を使用していない旧実装
- **新関数 `renderVectorStats()` の実装**
  - `computeVectorStats(logs)` を使用してベクトル統計を計算
  - `window.VectorMath.cosineSimilarity` のみを使用（vector_math.js に統合済み）
  - 未定義変数 `actualVector`, `idealVector` を一切使用しない
  - 軸ごとに直接 `avg[axis]` から値を取得
- **関数呼び出しの統一**
  - `renderAll()` 内: `renderVectorAnalysis()` → `renderVectorStats()` に変更（4箇所）
  - `renderAllWithProject()` 内: 重複を削除（`renderAll()` 内で既に呼ばれるため）
- **window.AnalysisDashboard への公開**
  - `renderVectorAnalysis: renderVectorAnalysis` → `renderVectorStats: renderVectorStats` に変更

### 削除したコード
- **削除した関数:**
  - `renderVectorAnalysis(projectData, logs)` - 旧実装（削除済み）
- **削除した変数参照:**
  - `actualVector` - 未定義変数（820行目、830行目で参照されていた）
  - `idealVector` - 未定義変数（旧実装で使用されていた）
- **削除した処理:**
  - ベクトル距離を直接計算する旧関数（削除済み）

### 残存するベクトル系処理（統一後）
- ✅ `computeVectorStats(logs)` - ベクトル統計計算
- ✅ `renderVectorStats(projectData, logs)` - ベクトル統計レンダリング
- ✅ `window.VectorMath.cosineSimilarity` - コサイン類似度計算（vector_math.js）

## 改善の詳細（旧ベクトル解析コード削除）

### 構造統一の効果
1. **`computeVectorStats()` を使用**
   - ログからベクトルを集計する処理を `computeVectorStats()` に統一
   - 重複コードを排除
   - 一貫性のあるデータ処理

2. **`vector_math.js` のみを使用**
   - `window.VectorMath.cosineSimilarity` のみを使用
   - ベクトル距離計算などの旧関数を削除
   - ES module 禁止ルールに準拠

3. **未定義変数の排除**
   - `actualVector`, `idealVector` を一切使用しない
   - 配列 `idealArr`, `actualArr` を直接使用
   - バグの根本原因を解消

4. **UI表示の改善**
   - 軸ごとに直接 `avg[axis]` から値を取得して表示
   - 未定義変数によるエラーを防止
   - より安全で保守性の高いコード

### コード品質の向上
- **重複コードの削除**: ベクトル集計処理を `computeVectorStats()` に統一
- **バグの解消**: 未定義変数 `actualVector`, `idealVector` の参照を削除
- **構造の明確化**: ベクトル系処理を `computeVectorStats()` と `renderVectorStats()` に集約
- **保守性の向上**: 一貫性のあるコード構造により、将来の変更が容易に

## 動作確認項目

以下の動作を確認してください：

1. ✅ `renderVectorStats()` が正しく動作する
2. ✅ `computeVectorStats()` を使用している
3. ✅ `window.VectorMath.cosineSimilarity` のみを使用している
4. ✅ 未定義変数 `actualVector`, `idealVector` の参照がない
5. ✅ ベクトル分析タブで正しく表示される
6. ✅ コンソールエラーがゼロである
7. ✅ `renderVectorAnalysis` の参照が0件（すべて削除済み）
