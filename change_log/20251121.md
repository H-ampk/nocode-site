...
## 🔄 11. gameData の統一（window.gameData への統合）

### 修正したファイル
- `src/editor/modules/core/state.js` - `getGameData()` と `setGameData()` が `window.gameData` を使用するように修正
- `editor.js` - 主要な関数で `gameData` を `window.gameData` に変更
- `src/editor/modules/project/save.js` - `loadProjectFromId()` で `window.gameData` を初期化するように修正

### 問題点
- `gameData` が複数の場所で定義・参照されており、データの不整合が発生する可能性があった
- モジュール側の `state.js` とレガシー `editor.js` で異なる `gameData` 変数を使用していた
- `editor_init.js` で設定した `window.gameData` がモジュール側で正しく参照されていなかった

### 修正内容
- **`state.js` の `getGameData()` と `setGameData()` を修正**: 
  - `getGameData()` が `window.gameData` を返すように変更
  - `setGameData()` が `window.gameData` を設定するように変更
  - `window.gameData` が存在しない場合は自動初期化
- **`editor.js` の主要な関数を修正**:
  - `gameData` 宣言を `window.gameData` に変更
  - `updateNodeList()`, `updateEditor()`, `addQuestion()`, `addDiagnosticQuestion()`, `addResult()`, `loadTemplate()` などの主要関数で `window.gameData` を使用
  - すべての `gameData.questions`, `gameData.results`, `gameData.startNode` 参照を `window.gameData` に統一
- **`loadProjectFromId()` を修正**:
  - プロジェクト読み込み時に `window.gameData` を直接設定
  - メタデータ統合時に `window.gameData` を更新

### 効果
- `gameData` が `window.gameData` に統一され、データの不整合が解消された
- モジュール側とレガシー側が同じデータソース（`window.gameData`）を参照するようになった
- `editor_init.js` で設定したデータがすべてのモジュールで正しく参照されるようになった
- エディタの描画が正しく動作し、ノードクリック時の編集画面表示が正常化された
...
18. `src/editor/modules/core/state.js` - `getGameData()` と `setGameData()` が `window.gameData` を使用するように修正
19. `editor.js` - 主要な関数で `gameData` を `window.gameData` に統一
20. `src/editor/modules/project/save.js` - `loadProjectFromId()` で `window.gameData` を初期化するように修正
...
### gameData の統一
- [x] `state.js` の `getGameData()` と `setGameData()` が `window.gameData` を使用
- [x] `editor.js` の主要な関数で `window.gameData` を使用
- [x] モジュール側とレガシー側が同じデータソースを参照
- [x] データの不整合が解消された
- [x] エディタの描画が正しく動作する
...
10. **データソースの統一**: `gameData` を `window.gameData` に統一し、モジュール側とレガシー側が同じデータを参照するように改善
